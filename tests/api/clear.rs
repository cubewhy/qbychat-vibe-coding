...
async fn dm_user_can_clear_all_messages() -> anyhow::Result<()> {
    ...
    // send messages
    for _ in 0..3 {
        let _ = app.client.post(format!("{}/api/chats/{}/messages", app.address, dm))
            .bearer_auth(&token_a)
            .json(&json!({"content":"hi"}))
            .send().await?;
    }
    let before: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM messages WHERE chat_id = $1").bind(&dm).fetch_one(&app.pool).await?;
    assert_eq!(before, 3);

    // clear as B
    let res = app.client.post(format!("{}/api/chats/{}/clear_messages", app.address, dm))
        .bearer_auth(&token_b)
        .send().await?;
    assert!(res.status().is_success());
    let after: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM messages WHERE chat_id = $1").bind(&dm).fetch_one(&app.pool).await?;
    assert_eq!(after, 0);
...
async fn group_clear_only_owner_or_admin() -> anyhow::Result<()> {
    ...
    let token_owner = app.client.post(format!("{}/api/register", app.address))
        ...
        .json::<serde_json::Value>().await?
        .get("token").and_then(|v| v.as_str()).unwrap().to_string();
    let token_member = app.client.post(format!("{}/api/register", app.address))
    ...
    // some messages
    for _ in 0..2 {
        let _ = app.client.post(format!("{}/api/chats/{}/messages", app.address, gid))
            .bearer_auth(&token_owner)
            .json(&json!({"content":"hi"}))
            .send().await?;
    }
    ...
    // member cannot clear
    let res = app.client.post(format!("{}/api/chats/{}/clear_messages", app.address, gid))
    ...
    let res = app.client.post(format!("{}/api/chats/{}/clear_messages", app.address, gid))
        ...
        .send().await?;
    assert!(res.status().is_success());
    let c: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM messages WHERE chat_id = $1").bind(&gid).fetch_one(&app.pool).await?;
    assert_eq!(c, 0);
