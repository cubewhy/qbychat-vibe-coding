...
async fn next_text(ws: &mut WsStream) -> anyhow::Result<String> {
    ...
    loop {
        ...
        match item {
            ...
            Ok(WsMessage::Binary(_)) | Ok(WsMessage::Ping(_)) | Ok(WsMessage::Pong(_)) | Ok(WsMessage::Frame(_)) => continue,
...
async fn websocket_send_and_receive() -> anyhow::Result<()> {
    let app = match TestApp::spawn().await { Ok(a) => a, Err(e) => { eprintln!("skipping ws test: {}", e); return Ok(()); } };

    // register users
    let token_alice = app.client.post(format!("{}/api/register", app.address))
        .json(&json!({"username":"alice","password":"secretpw"}))
        .send().await?
        .json::<serde_json::Value>().await?
        .get("token").and_then(|v| v.as_str()).unwrap().to_string();
    let token_bob = app.client.post(format!("{}/api/register", app.address))
    ...
    // alice send message
    let payload = json!({"type": "send_message", "chat_id": chat_id, "content": "hello"}).to_string();
