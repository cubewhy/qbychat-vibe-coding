...
async fn members_created_and_unread_counts() -> anyhow::Result<()> {
    ...
    let unread = app
        .client
        .get(format!("{}/api/chats/{}/unread_count", app.address, dm_id))
        .bearer_auth(&token_b)
        .send()
        .await?
        .json::<serde_json::Value>()
    ...
    let _ = app
        .client
        .post(format!("{}/api/messages/{}/delete", app.address, m2))
        .bearer_auth(&token_a)
        .send()
        .await?;
    let unread = app
...
async fn member_note_crud_and_leave() -> anyhow::Result<()> {
    ...
    let res = app
        .client
        .post(format!("{}/api/chats/{}/member/note", app.address, dm))
        .bearer_auth(&token_a)
    ...
    let got = app
        .client
        .get(format!("{}/api/chats/{}/member/note", app.address, dm))
        .bearer_auth(&token_a)
        .send()
        .await?
        .json::<serde_json::Value>()
    ...
    let res = app
        .client
        .delete(format!("{}/api/chats/{}/member/note", app.address, dm))
        .bearer_auth(&token_a)
        .send()
        .await?;
    assert!(res.status().is_success());
    ...
    let res = app
        .client
        .post(format!("{}/api/chats/{}/leave", app.address, dm))
        .bearer_auth(&token_a)
        .send()
        .await?;
    assert!(res.status().is_success());
