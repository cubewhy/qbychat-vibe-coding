...
async fn send_with_attachment_and_reply_and_list() -> anyhow::Result<()> {
    ...
    // upload a file
    let part = reqwest::multipart::Part::bytes(b"data".to_vec()).file_name("a.bin").mime_str("application/octet-stream").unwrap();
    let form = reqwest::multipart::Form::new().part("f1", part);
    let files = app.client.post(format!("{}/api/files", app.address))
        .bearer_auth(&token_a)
        .multipart(form)
        .send().await?
        .json::<serde_json::Value>().await?;
    ...
    // send base message
    let m1 = app.client.post(format!("{}/api/chats/{}/messages", app.address, chat_id))
        .bearer_auth(&token_a)
    ...
    let m2 = app.client.post(format!("{}/api/chats/{}/messages", app.address, chat_id))
        .bearer_auth(&token_b)
    ...
    let cnt: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM message_attachments").fetch_one(&app.pool).await?;
    assert_eq!(cnt, 1);

    // list messages -> should include attachments and reply_to
    let list = app.client.get(format!("{}/api/chats/{}/messages", app.address, chat_id))
        .bearer_auth(&token_a)
        .send().await?
        .json::<serde_json::Value>().await?;
