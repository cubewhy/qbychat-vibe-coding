...
async fn promote_demote_remove_and_mute_flow() -> anyhow::Result<()> {
    ...
    // add alice
    app.client.post(format!("{}/api/chats/{}/participants", app.address, chat_id))
        .bearer_auth(&token_owner)
        .json(&json!({"username":"alice"}))
        .send().await?;

    // promote alice to admin
    let res = app.client.post(format!("{}/api/chats/{}/admins", app.address, chat_id))
        .bearer_auth(&token_owner)
        .json(&json!({"username":"alice"}))
        .send().await?;
    ...
    // mute alice 1 minute
    let res = app.client.post(format!("{}/api/chats/{}/mute", app.address, chat_id))
        .bearer_auth(&token_owner)
    ...
    // unmute alice
    let res = app.client.post(format!("{}/api/chats/{}/unmute", app.address, chat_id))
        .bearer_auth(&token_owner)
        .json(&json!({"username":"alice"}))
        .send().await?;
    assert!(res.status().is_success());
    ...
    // demote alice
    let res = app.client.post(format!("{}/api/chats/{}/admins/remove", app.address, chat_id))
        .bearer_auth(&token_owner)
        .json(&json!({"username":"alice"}))
        .send().await?;
    assert!(res.status().is_success());
    ...
    // remove member
    let res = app.client.post(format!("{}/api/chats/{}/remove", app.address, chat_id))
        .bearer_auth(&token_owner)
        .json(&json!({"username":"alice"}))
        .send().await?;
    assert!(res.status().is_success());
