...
async fn chat_list_with_options_and_notify() -> anyhow::Result<()> {
    ...
    // list with options
    let list = app.client.get(format!("{}/api/chats?include_unread=true&include_first=true", app.address))
        .bearer_auth(&token_b)
        .send().await?
        .json::<serde_json::Value>().await?;
    ...
    let res = app.client.post(format!("{}/api/chats/{}/member/notify", app.address, dm))
        .bearer_auth(&token_b)
    ...
    // get notify settings
    let got = app.client.get(format!("{}/api/chats/{}/member/notify", app.address, dm))
        .bearer_auth(&token_b)
        .send().await?
        .json::<serde_json::Value>().await?;
    ...
    // mentions list
    let mention_ids = app.client.get(format!("{}/api/chats/{}/member/mentions", app.address, dm))
        .bearer_auth(&token_b)
        .send().await?
        .json::<serde_json::Value>().await?;
    ...
    // clear mentions
    let res = app.client.delete(format!("{}/api/chats/{}/member/mentions", app.address, dm))
        .bearer_auth(&token_b)
        .send().await?;
    assert!(res.status().is_success());
