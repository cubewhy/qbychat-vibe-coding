...
use chrono::Utc;
use jsonwebtoken::{EncodingKey, Header as JwtHeader, DecodingKey, Validation, Algorithm};
...
pub fn make_token(user_id: Uuid, secret: &str) -> actix_web::Result<String> {
    ...
    jsonwebtoken::encode(&JwtHeader::default(), &claims, &EncodingKey::from_secret(secret.as_bytes())).map_err(internal_err)
}

pub fn decode_token(token: &str, secret: &str) -> Result<Uuid, jsonwebtoken::errors::Error> {
    let data = jsonwebtoken::decode::<Claims>(token, &DecodingKey::from_secret(secret.as_bytes()), &Validation::new(Algorithm::HS256))?;
...
impl actix_web::FromRequest for AuthUser {
    ...
    fn from_request(req: &actix_web::HttpRequest, _payload: &mut actix_web::dev::Payload) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            let uid = crate::auth::decode_token(&token, &secret).map_err(|_| actix_web::error::ErrorUnauthorized("invalid token"))?;
            Ok(AuthUser(uid))
        })
    }
}

pub fn internal_err<E: std::fmt::Debug>(e: E) -> actix_web::Error { actix_web::error::ErrorInternalServerError(format!("{:?}", e)) }
pub fn conflict_or_internal<E: std::fmt::Debug>(e: E) -> actix_web::Error { actix_web::error::ErrorConflict(format!("{:?}", e)) }
