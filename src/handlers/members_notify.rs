...
async fn ensure_member(state: &AppState, chat_id: Uuid, user_id: Uuid) -> Result<bool, actix_web::Error> {
    let is_member = sqlx::query_scalar::<_, Option<i64>>("SELECT 1 FROM chat_participants WHERE chat_id = $1 AND user_id = $2")
        .bind(chat_id).bind(user_id).fetch_one(&state.pool).await.map_err(internal_err)?.is_some();
    Ok(is_member)
}

#[get("/api/chats/{chat_id}/member/notify")] 
pub async fn get_notify(state: web::Data<AppState>, path: web::Path<Uuid>, user: AuthUser) -> actix_web::Result<HttpResponse> {
    let chat_id = path.into_inner();
    if !ensure_member(&state, chat_id, user.0).await? { return Ok(HttpResponse::Forbidden().finish()); }
    ...
    Ok(HttpResponse::Ok().json(NotifyResp { mute_forever, mute_until, notify_type }))
}

#[post("/api/chats/{chat_id}/member/notify")] 
pub async fn set_notify(state: web::Data<AppState>, path: web::Path<Uuid>, user: AuthUser, req: web::Json<NotifyReq>) -> actix_web::Result<HttpResponse> {
    let chat_id = path.into_inner();
    if !ensure_member(&state, chat_id, user.0).await? { return Ok(HttpResponse::Forbidden().finish()); }
...
pub async fn get_mentions(state: web::Data<AppState>, path: web::Path<Uuid>, user: AuthUser) -> actix_web::Result<HttpResponse> {
    let chat_id = path.into_inner();
    if !ensure_member(&state, chat_id, user.0).await? { return Ok(HttpResponse::Forbidden().finish()); }
    let ids: Option<Vec<Uuid>> = sqlx::query_scalar("SELECT mention_message_ids FROM chat_members WHERE chat_id = $1 AND user_id = $2")
        .bind(chat_id).bind(user.0).fetch_optional(&state.pool).await.map_err(internal_err)?;
    Ok(HttpResponse::Ok().json(serde_json::json!({"message_ids": ids.unwrap_or_default()})))
}

#[delete("/api/chats/{chat_id}/member/mentions")] 
pub async fn clear_mentions(state: web::Data<AppState>, path: web::Path<Uuid>, user: AuthUser) -> actix_web::Result<HttpResponse> {
    let chat_id = path.into_inner();
    if !ensure_member(&state, chat_id, user.0).await? { return Ok(HttpResponse::Forbidden().finish()); }
    sqlx::query("UPDATE chat_members SET mention_message_ids = '{}' WHERE chat_id = $1 AND user_id = $2")
        .bind(chat_id).bind(user.0).execute(&state.pool).await.map_err(internal_err)?;
    Ok(HttpResponse::Ok().finish())
}
