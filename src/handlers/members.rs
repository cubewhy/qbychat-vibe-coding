...
#[derive(Deserialize)]
pub struct NoteReq { pub note: String }
...
pub async fn get_note(state: web::Data<AppState>, path: web::Path<Uuid>, user: AuthUser) -> actix_web::Result<HttpResponse> {
    let chat_id = path.into_inner();
    if !ensure_member(&state, chat_id, user.0).await? { return forbidden(); }
    let note: Option<String> = sqlx::query_scalar("SELECT note FROM chat_members WHERE chat_id = $1 AND user_id = $2")
...
pub async fn set_note(state: web::Data<AppState>, path: web::Path<Uuid>, user: AuthUser, req: web::Json<NoteReq>) -> actix_web::Result<HttpResponse> {
    let chat_id = path.into_inner();
    if !ensure_member(&state, chat_id, user.0).await? { return forbidden(); }
...
pub async fn delete_note(state: web::Data<AppState>, path: web::Path<Uuid>, user: AuthUser) -> actix_web::Result<HttpResponse> {
    let chat_id = path.into_inner();
    if !ensure_member(&state, chat_id, user.0).await? { return forbidden(); }
