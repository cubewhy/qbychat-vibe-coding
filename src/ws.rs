...
#[get("/ws")]
pub async fn ws_route(state: web::Data<AppState>, req: HttpRequest, body: web::Payload) -> actix_web::Result<impl Responder> {
...
async fn handle_ws_text(state: &AppState, user_id: Uuid, txt: &str) -> anyhow::Result<()> {
    ...
    match msg {
        ClientWsMsg::SendMessage { chat_id, content } => {
            ...
            for p in participants { if let Some(tx) = state.clients.get(&p.user_id) { let _ = tx.send(server_msg.clone()); } }
        }
    }
    Ok(())
}
